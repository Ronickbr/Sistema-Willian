...
-- Atualizado com todos os campos utilizados no sistema
...
CREATE TABLE transportadoras (
    ...
    email VARCHAR(255),
    endereco TEXT,
    peso_ate_50kg DECIMAL(10,2) DEFAULT 0,
    peso_ate_100kg DECIMAL(10,2) DEFAULT 0,
    peso_ate_150kg DECIMAL(10,2) DEFAULT 0,
    peso_ate_200kg DECIMAL(10,2) DEFAULT 0,
    peso_ate_300kg DECIMAL(10,2) DEFAULT 0,
    frete_por_tonelada DECIMAL(10,2) DEFAULT 0,
    frete_minimo DECIMAL(10,2) DEFAULT 0,
    pedagio DECIMAL(10,2) DEFAULT 0,
    frete_valor_percentual DECIMAL(5,2) DEFAULT 0,
    fator_peso_cubico DECIMAL(10,2) DEFAULT 0,
    ativo BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Criar índices para transportadoras
...
CREATE INDEX idx_cotacoes_data_cotacao ON cotacoes(data_cotacao);

-- Adicionar foreign key para cotacao_id em pedidos após criar tabela cotacoes
ALTER TABLE pedidos ADD FOREIGN KEY (cotacao_id) REFERENCES cotacoes(id);
...
INSERT INTO transportadoras (nome, cnpj, telefone, email, endereco, peso_ate_50kg, peso_ate_100kg, peso_ate_150kg, peso_ate_200kg, peso_ate_300kg, frete_por_tonelada, frete_minimo, pedagio, frete_valor_percentual, fator_peso_cubico) VALUES 
('Transportadora Exemplo', '12.345.678/0001-90', '(11) 1234-5678', 'contato@exemplo.com', 'Rua das Flores, 123 - São Paulo/SP', 15.00, 25.00, 35.00, 45.00, 55.00, 120.00, 50.00, 8.00, 2.5, 300.00),
('Expresso Rápido', '98.765.432/0001-10', '(21) 9876-5432', 'vendas@expressorapido.com', 'Av. Principal, 456 - Rio de Janeiro/RJ', 18.00, 28.00, 38.00, 48.00, 58.00, 130.00, 60.00, 9.00, 3.0, 320.00),
('Logística Total', '11.222.333/0001-44', '(31) 5555-1234', 'comercial@logisticatotal.com', 'Rua do Comércio, 789 - Belo Horizonte/MG', 12.00, 22.00, 32.00, 42.00, 52.00, 110.00, 45.00, 7.50, 2.0, 280.00);

-- Inserir pedido exemplo
INSERT INTO pedidos (numero_pedido, numero_picking, cliente_nome, origem, destino, peso, valor_mercadoria, observacoes, usuario_id) VALUES 
('PED-2024-001', 'PICK-001', 'Cliente Exemplo Ltda', 'São Paulo - SP', 'Rio de Janeiro - RJ', 25.50, 1500.00, 'Produtos diversos para entrega', 1),
('PED-2024-002', 'PICK-002', 'Empresa ABC', 'Belo Horizonte - MG', 'Salvador - BA', 18.30, 2200.00, 'Equipamentos eletrônicos', 1);
...
INSERT INTO cotacoes (pedido_id, transportadora_id, numero_nota_fiscal, valor_nota_fiscal, peso_nota_fiscal, valor_frete, valor_frete_calculado, cubagem_total, prazo_entrega, status) VALUES 
(1, 1, 'NF-001', 1500.00, 25.50, 85.00, 82.50, 0.1350, 3, 'aprovada'),
(1, 2, 'NF-001', 1500.00, 25.50, 92.00, 89.20, 0.1350, 2, 'pendente'),
(2, 1, 'NF-002', 2200.00, 18.30, 120.00, 115.80, 0.1050, 5, 'pendente');
